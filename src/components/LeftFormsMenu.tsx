...
export const LEFT_MENU_WIDTH = 260;
const NAVBAR_HEIGHT = 80; // Altura exacta de tu NavBar azul
...
const STORAGE_KEYS = {
  EVAL: "leftMenu.evalEcoOpen",
  REF: "leftMenu.referenciasOpen",
  PROP: "leftMenu.propuestaOpen",
  HOJA: "leftMenu.hojaSupervisionOpen",
};

type MenuItem = { label: string; path: string; };
type MenuGroup = { label: string; basePath: string; children: MenuItem[]; };

const FORM_MENU_ITEMS: MenuItem[] = [
  { label: "INICIO", path: "/" },
  { label: "PRECALIFICACION", path: "/forms/precalificacion" },
  { label: "SOLICITUD", path: "/forms/solicitud" },
  { label: "NUEVA GESTIÓN", path: "/forms/gestion" },
];
...
export const LeftFormsMenu: React.FC = () => {
  ...
  const [evalOpen, setEvalOpen] = React.useState(() => localStorage.getItem(STORAGE_KEYS.EVAL) === "true");
  const [refOpen, setRefOpen] = React.useState(() => localStorage.getItem(STORAGE_KEYS.REF) === "true");
  const [propOpen, setPropOpen] = React.useState(() => localStorage.getItem(STORAGE_KEYS.PROP) === "true");
  const [hojaOpen, setHojaOpen] = React.useState(() => localStorage.getItem(STORAGE_KEYS.HOJA) === "true");

  React.useEffect(() => {
    localStorage.setItem(STORAGE_KEYS.EVAL, String(evalOpen));
    localStorage.setItem(STORAGE_KEYS.REF, String(refOpen));
    localStorage.setItem(STORAGE_KEYS.PROP, String(propOpen));
    localStorage.setItem(STORAGE_KEYS.HOJA, String(hojaOpen));
  }, [evalOpen, refOpen, propOpen, hojaOpen]);

  const renderGroup = (group: MenuGroup, open: boolean, onToggle: () => void) => {
    return (
      <>
        <ListItemButton onClick={onToggle} sx={{ mb: 0.5, borderRadius: 1, bgcolor: open ? OPEN_BG : "transparent", "&:hover": { bgcolor: OPEN_BG } }}>
          <ListItemText primary={group.label} primaryTypographyProps={{ fontWeight: 700, fontSize: 12, color: "rgba(0,0,0,0.7)" }} />
          <KeyboardArrowRightIcon sx={{ fontSize: 18, transition: "transform 0.3s", transform: open ? "rotate(90deg)" : "rotate(0deg)", color: "rgba(0,0,0,0.5)" }} />
        </ListItemButton>
        <Collapse in={open} timeout="auto" unmountOnExit>
          <List sx={{ pl: 1 }}>
            {group.children.map((child) => {
              const selected = location.pathname === child.path;
              return (
                <ListItemButton key={child.path} selected={selected} onClick={() => navigate(child.path)} sx={{ mb: 0.5, borderRadius: 1, "&.Mui-selected": { bgcolor: "transparent", "& .MuiTypography-root": { color: MENU_SELECTED, fontWeight: 700 } }, "&:hover": { bgcolor: OPEN_BG } }}>
                  <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: DOT_COLOR, mr: 1.5, flexShrink: 0 }} />
                  <ListItemText primary={child.label} primaryTypographyProps={{ fontSize: 12, fontWeight: 500, color: selected ? MENU_SELECTED : "rgba(0,0,0,0.65)" }} />
                </ListItemButton>
              );
            })}
          </List>
    ...
    >
      <List sx={{ p: 0 }}>
        {FORM_MENU_ITEMS.map((item) => {
          const selected = location.pathname === item.path;
          return (
            <ListItemButton key={item.path} selected={selected} onClick={() => navigate(item.path)} sx={{ mb: 0.5, borderRadius: 1, "&.Mui-selected": { bgcolor: "transparent", "& .MuiTypography-root": { color: MENU_SELECTED } }, "&:hover": { bgcolor: OPEN_BG } }}>
              <ListItemText primary={item.label} primaryTypographyProps={{ fontWeight: 700, fontSize: 12, color: selected ? MENU_SELECTED : "rgba(0,0,0,0.7)" }} />
            </ListItemButton>
          );
        })}
        {renderGroup(EVALUACION_ECONOMICA_GROUP, evalOpen, () => setEvalOpen(!evalOpen))}
        {renderGroup(REFERENCIAS_GROUP, refOpen, () => setRefOpen(!refOpen))}
        {renderGroup(PROPUESTA_GROUP, propOpen, () => setPropOpen(!propOpen))}
        {renderGroup(HOJA_SUPERVISION_GROUP, hojaOpen, () => setHojaOpen(!hojaOpen))}
      </List>
    </Box>
  );
};