...
export const PrecalificationFormContent = () => {
    ...
    const [form] = useState<PrecalificacionFormState>({
        dominioProspecto: "Propio", documento: "358437", consultar: "Cliente Existente",
        primerNombre: "JOSE", tercerNombre: "", segundoApellido: "LON",
        consultarCompartir: "Si", telefonoCelular: "78152020", numeroDocumentoPrincipal: "043649194",
        calificacionInterna: "P7", equifax: "equifax", generarinstancia: "ok",
        busqueda: "Cuenta Cliente", tipoSolicitud: "Nuevo", cuentaCliente: "358437",
        segundoNombre: "MARIO", primerApellido: "MARQUEN", apellidoCasada: "",
        datosPersonales: "Si", correoElectronico: "user.user@gmail.com",
        nit: "06020810901015", ssf: "Solicitud sin datos", infored: "infored",
    });
        ...
        <Paper elevation={0} sx={{ p: 4, borderRadius: 3, border: "1px solid #E5E7EB" }}>
            ...
            <Box sx={{ display: "flex", gap: 4 }}>
                <Stack spacing={2.5} sx={{ flex: 1 }}>
                    {[
                        { id: "dominioProspecto", label: "Dominio de prospecto" },
                        { id: "documento", label: "Documento" },
                        { id: "consultar", label: "Consultar" },
                        { id: "primerNombre", label: "Primer nombre" },
                        { id: "tercerNombre", label: "Tercer nombre" },
                        { id: "segundoApellido", label: "Segundo apellido" },
                        { id: "consultarCompartir", label: "Consultar/Compartir" },
                        { id: "telefonoCelular", label: "Teléfono celular" },
                        { id: "numeroDocumentoPrincipal", label: "Número de Documento Principal" },
                        { id: "calificacionInterna", label: "Calificación interna" },
                        { id: "equifax", label: "Equifax" },
                        { id: "generarinstancia", label: "Generar instancia" },
                    ].map((f) => (
                        <FormInput key={f.id} identifier={f.id} label={f.label} value={form[f.id as keyof PrecalificacionFormState]} onValueChange={() => {}} disabled textFieldProps={{ sx: inputStyleSx }} />
                    ))}
                </Stack>
                <Stack spacing={2.5} sx={{ flex: 1 }}>
                    {[
                        { id: "busqueda", label: "Búsqueda" },
                        { id: "tipoSolicitud", label: "Tipo solicitud" },
                        { id: "cuentaCliente", label: "Cuenta Cliente" },
                        { id: "segundoNombre", label: "Segundo nombre" },
                        { id: "primerApellido", label: "Primer apellido" },
                        { id: "apellidoCasada", label: "Apellido casada" },
                        { id: "datosPersonales", label: "Datos personales" },
                        { id: "correoElectronico", label: "Correo electrónico" },
                        { id: "nit", label: "NIT" },
                        { id: "ssf", label: "SSF" },
                        { id: "infored", label: "Infored" },
                    ].map((f) => (
                        <FormInput key={f.id} identifier={f.id} label={f.label} value={form[f.id as keyof PrecalificacionFormState]} onValueChange={() => {}} disabled textFieldProps={{ sx: inputStyleSx }} />
                    ))}
                </Stack>
...
// Mantenemos este por si se accede por URL directa
export const PrecalificationForm = () => (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        <Box sx={{ display: "flex", flexGrow: 1, overflow: "hidden", bgcolor: "#F8F9FA" }}>
            ...
            <Box component="main" sx={{ flexGrow: 1, overflowY: "auto", p: 3 }}>
                <Typography variant="h5" fontWeight={700} sx={{ mb: 3 }}>Precalificación</Typography>
                <PrecalificationFormContent />
            </Box>
        </Box>
    </Box>
);